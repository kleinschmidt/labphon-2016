...
> * Speech perception as __inference under uncertainty__ at __multiple scales__
> * Framework to link __production data__ with __perception__
> * __Constraints__ on rapid adaptation via distributional learning
> * Probe listeners' __subjective beliefs__ about an unfamiliar talker
...
plot_particles_lhood <- function(particles, x_to_plot_at, true_particle = NA) {
  ...
  particle_pdfs %>%
    ...
    geom_line(aes(x=x_pdf, y=particle_lhood, group=p_idx,
                  color='Hypotheses'), alpha=0.05, size=line_weight) +
    geom_line(data=particle_pdfs_marginal, size=line_weight,
  ...
  if (!missing(true_particle)) {
    ...
    p <- p + 
      geom_line(data = true_lhood, size=line_weight,
                aes(x=x_pdf, y=lhood, color='Truth')) +
...
plot_particles_id <- function(particles, x_to_plot_at, true_particle = NA) {
  ...
  particle_ids %>%
    ...
    geom_line(alpha=0.05, size=line_weight,
  ...
  if (!missing(true_particle)) {
    ...
    p <- p +
      geom_line(data = true_id, aes(y = id, color='Truth'), size=line_weight)
  }
...
okay_particles <- readRDS('okay_particles.rds')
...
flat_particles <- readRDS('flat_particles.rds')
...
mis_particles <- readRDS('mis_particles.rds')
...
> * An ideal adapter __won't always adapt__
> * Adaptation is __constrained__, guided by __actual variability__ across talkers (as experienced by each listener).
...
<div class="notes">
* Design is based on clayards 2008
</div>
...
<span class="cite">[Clayards et al., 2008]</span>
...
library(rstan)
mod_lapsing <- stan(file.path(devtools::inst('beliefupdatr'),
                              'exec', 'conj_id_lapsing_fit.stan'))
...
```{r inferred-prior-vs-kronrod, fig.width=10, fig.height=4, out.width='100%'}
...
buckeye_prior_lhood <-
  votcorpora::vot %>%
  filter(place == 'lab') %>%
  group_by(source, phoneme, voicing, prevoiced, subject) %>%
  summarise_each(funs(mean, sd), vot) %>%
  summarise_each(funs(mean), mean, sd) %>%
  filter(source == 'buckeye') %>%
  rename(category = phoneme) %>%
  stats_to_lhood(noise_sd=0)

all_priors <- buckeye_prior_lhood %>%
  mutate(source = "Buckeye") %>%
...
<span class="cite">[Wedel, _in prep_]</span>

## Inferred prior beliefs

```{r inferred-prior-vs-goldrick-2, fig.width=10, fig.height=4, out.width='100%'}
...
```{r inferred-prior-vs-goldrick-3, fig.width=10, fig.height=4, out.width='100%'}
...
<span class="cite">[Wedel, _in prep_; Goldrick, Vaughn, & Murphy, 2013]</span>
